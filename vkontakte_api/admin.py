...
class GenericRelationListFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        return [('%s%s%s' % (getattr(instance, self.ct_field_name).id, self.separator,
                             getattr(instance, self.id_field_name)), getattr(instance, self.field_name))
                for instance in model_admin.model.objects.order_by().distinct(self.ct_field_name,self.id_field_name)]
